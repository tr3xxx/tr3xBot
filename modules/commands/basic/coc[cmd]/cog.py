...
from utils.coc_login import email,password
...
class coc(commands.Cog):
    ...
    async def coc(self,ctx, arg: str = None):
        ...
        await ctx.send('This Command is not working at the moment due Issues from Supercell')
        
        if 0!=0:

            cocclient = await coc.login(email(),password())
