...
class eThotHub(commands.Cog):

    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @commands.command(pass_context=True,aliases=['ethothub'])
    async def eThotHub(self,ctx):
        async with ctx.typing():
            
            if ctx.channel.is_nsfw():
                while True:
                    subreddit = await reddit.subreddit("eThotHub")
